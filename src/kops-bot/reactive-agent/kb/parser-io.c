 ...
 *         0 - otherwise.
 */
int initIOContext(IOContext *io_ctx, KBContext *kb_ctx)
{
 ...
 * @param io_ctx - pointer to I/O context storage.
 */
void finalizeIOContext(IOContext *io_ctx)
{
 ...
 *         0 - otherwise.
 */
int printKBFiles(IOContext *io_ctx, KBContext *kb_ctx)
{
...
void printKBChoiceInfo(IOContext *io_ctx)
{
...
printRule(KBContext *ctx, Rule *rule, FILE *output)
{
    ...
    if (rule->positive != NULL || rule->negative != NULL) {
        ...
        indent(output, 1);
        fprintf(output, "rule = makeKBRule();\n");
        ...
        indent(output, 1);
        fprintf(output, "rule->premise_positive_variables = ");
        ...
        indent(output, 1);
        fprintf(output, "rule->premise_positive_percepts = ");
        ...
        indent(output, 1);
        fprintf(output, "rule->premise_negative_variables = ");
        ...
        indent(output, 1);
        fprintf(output, "rule->premise_negative_percepts = ");
        ...
        indent(output, 1);
        fprintf(output, "rule->conclusion_positive_variables = ");
        ...
        indent(output, 1);
        fprintf(output, "rule->conclusion_negative_variables = ");
...
printKBRules(KBContext *ctx, FILE *output)
{
    ...
    indent(output, 1);
    fprintf(output, "/* Rules of %s knowledge base. */\n", ctx->name);
    ...
    for (label = ctx->labels; label != NULL; label = label->next) {
        ...
        fprintf(output, "\n");
        indent(output, 1);
        fprintf(output, "/* Jump target: '%s' */\n", label->name);
        indent(output, 1);
        fprintf(output, "target = makeJumpTarget(%d);\n", label->id);
        indent(output, 1);
        fprintf(output, "addJumpTarget(kb, target);\n");
    }
    ...
    for (label = ctx->labels; label != NULL; label = label->next) {
        ...
        fprintf(output, "\n");
        indent(output, 1);
        fprintf(output, "/* Add rules for jump target '%s' (%d).*/\n", label->name, label->id);
        indent(output, 1);
        fprintf(output, "target = findJumpTarget(kb, %d);\n", label->id);
        ...
        for (rule = label->rule; rule != NULL; rule = rule->next) {
            ...
            if (rule->implies->cmd != NULL) {
                if (!strcmp(rule->implies->cmd->opcode, "CMD_JUMP")) {
                    ...
                    indent(output, 1);
                    fprintf(output, "/* Set jump target. */\n");
                    indent(output, 1);
                    fprintf(output, "setJumpTo(kb, rule, %d);\n", rule->implies->jumpTo->id);
                }
            }
        }
    }
    ...
    fprintf(output, "\n");
    indent(output, 1);
    fprintf(output, "/* Set start rule.*/\n");
    indent(output, 1);
    fprintf(output, "target = findJumpTarget(kb, %d);\n", label->id);
    indent(output, 1);
    fprintf(output, "setStartRule(kb, target->rule);\n");

    fprintf(output, "\n");
    indent(output, 1);
    fprintf(output, "/* All done. */\n");
...
int nextToken(Tokenizer *tokenizer, int peek)
{
    ...
    if (!tokenizer->peeked) {
        ...
        if (token == token_name) {
            index = 0;
            while (isalnum(tokenizer->buffer[tokenizer->cursor + index]) || tokenizer->buffer[tokenizer->cursor + index] == '_' || tokenizer->buffer[tokenizer->cursor + index] == '-' || tokenizer->buffer[tokenizer->cursor + index] == '.') {
...
int loadKBRules(KBContext *ctx, char *filename)
{
