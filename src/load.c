...
Uint8 loadconfig()
{
    ...
    fclose(fp);
    snd = s_sdevice;
    if ((vmaj != versmaj) || (vmin != versmin) || (players == 0) || (players > 4) || (parallaxdraw > 1) || (slots < 2) || (bots == 0) || (bots > 4) || (slots > 16)) {
        return (0);
    }
    if ((s_sdevice > 1) || (s_samprate > 44100) || /*(s_interp>1)|| */ (s_shortchn == 0) ||
        (s_shortchn > 16) || (s_longchn == 0) || (s_longchn > 16) || (s_musvol > 64) || (s_effvol > 64) || s_samprate < 8000) {
        return (0);
    }
    return (1);
}
...
#define MAXLEVELREVISION 1
    ...
    for (b = levh - 1; b >= 0; b--) {
        for (a = levw - 1; a >= 0; a--) {
            if ((*(level + b * levw + a) >= 10) && (*(level + b * levw + a) <= 14)) {
                base[w].x = (short)a;
                base[w].y = (short)b;
...
{
    Uint8 fontstr[FONTCHARS] = {"ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"
                                "()%#@$!\"/\\+-_?=*.,><:;\345\344\366\260\261\262\333\372\371\376 "};
    Uint8 bigfontstr[BIGFONTCHRS] =
        {"ABCDEFGHIJKLMNOPQRSTUVWXYZ\345\344\3660123456789"
         "!\"?:.;,+-=@\334\337()/#%$\\_*<>\260\261\262\333\372\371\376 "};
    ...
    for (b = 0; b < boomanim[0].frames; b++) {
        for (a = 0; a < boomanim[0].h; a++) { /* 1st */
            memcpy(boomanim[0].pic + b * boomanim[0].siz +
                       a * boomanim[0].w,
                   unp + (a + 1) * gfxw + b * (boomanim[0].w + 1) + 18, boomanim[0].w);
        }
    }
    for (b = 0; b < boomanim[1].frames; b++) {
        for (a = 0; a < boomanim[1].h; a++) { /* 2nd */
            memcpy(boomanim[1].pic + b * boomanim[1].siz +
                       a * boomanim[1].w,
                   unp + (a + 18) * gfxw + b * (boomanim[1].w + 1) + 18, boomanim[1].w);
        }
    }
    for (b = 0; b < boomanim[2].frames; b++) {
        for (a = 0; a < boomanim[2].h; a++) { /* 3rd */
            memcpy(boomanim[2].pic + b * boomanim[2].siz +
                       a * boomanim[2].w,
                   unp + (a + 43) * gfxw + b * (boomanim[2].w + 1) + 18, boomanim[2].w);
        }
    }
    for (b = 0; b < 8; b++) {
        for (a = 0; a < boomanim[3].h; a++) { /* 4th/row1 */
            memcpy(boomanim[3].pic + b * boomanim[3].siz +
                       a * boomanim[3].w,
                   unp + (a + 76) * gfxw + b * (boomanim[3].w + 1) + 18, boomanim[3].w);
        }
    }
    for (b = 0; b < 8; b++) {
        for (a = 0; a < boomanim[3].h; a++) { /* 4th/row2 */
            memcpy(boomanim[3].pic + (b + 8) * boomanim[3].siz +
                       a * boomanim[3].w,
                   unp + (a + 125) * gfxw + b * (boomanim[3].w + 1) + 18, boomanim[3].w);
        }
    }
    for (b = 0; b < boomanim[4].frames; b++) {
        for (a = 0; a < boomanim[4].h; a++) { /* 5th */
            memcpy(boomanim[4].pic + b * boomanim[4].siz +
                       a * boomanim[4].w,
                   unp + (a + 193) * gfxw + b * (boomanim[4].w + 1) + 67, boomanim[4].w);
        }
    }
    for (b = 0; b < boomanim[5].frames; b++) {
        for (a = 0; a < boomanim[5].h; a++) { /* 6th */
            memcpy(boomanim[5].pic + b * boomanim[5].siz +
                       a * boomanim[5].w,
                   unp + (a + 218) * gfxw + b * (boomanim[5].w + 1) + 67, boomanim[5].w);
        }
    }
