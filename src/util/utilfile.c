...
/* returns 0 if success */
{
    int a, lib = UTIL_MAXLIBS;
    for (a = 0; a < UTIL_MAXLIBS; a++) {
        if (stricmp(filename, util_flib[a].libname) == 0) {
            lib = a;
        }
    }
    if (lib == UTIL_MAXLIBS) {
